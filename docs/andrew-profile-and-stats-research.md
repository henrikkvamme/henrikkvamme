...
The portrait is generated by `jp2a` 1.3.3 from a checked-in, logo-free crop. The command is captured in `scripts/generate_ascii_portrait.sh`, including the exact 50 by 34 character target and density palette. No character rows are hand-edited. The generated text is then embedded as literal escaped ASCII `<tspan>` rows, matching Andrew's delivery format while keeping Henrik's source and conversion reproducible.
