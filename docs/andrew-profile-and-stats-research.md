...
3. Prefer metrics that communicate shipped work: 12-month contributions, public merged pull requests, and stars on original public repositories. Keep the current-building section explicit and editorial rather than guessing it from recent repository activity.
...
Building
Devme ..................... Dev-stack supervisor for worktrees
Nixus ............ Nix configuration + synchronized agent skills
Sambu .............................. Co-living app for students
...
The public `Building` projects should link to their repositories in the surrounding README or via separate text below the image. SVG-in-image links are less predictable on GitHub than normal Markdown links. Private project names can remain unlinked for public visitors.

## Deterministic updater design

1. Generate the logo-free ASCII portrait from a checked-in source crop with a deterministic converter. Keep the generated text under version control so profile updates do not depend on image conversion.
...
5. Keep the editorial current-building list separate from computed GitHub statistics. This prevents repository recency from being mistaken for Henrik's actual priorities.
...
The portrait is generated by `jp2a` 1.3.3 from a checked-in, logo-free crop. The command is captured in `scripts/generate_ascii_portrait.sh`, including the exact 50-column maximum and density palette. Width-only conversion preserves the source aspect ratio instead of stretching it to a fixed row count. No character rows are hand-edited. The generated text is then embedded as literal escaped ASCII `<tspan>` rows, matching Andrew's delivery format while keeping Henrik's source and conversion reproducible.
